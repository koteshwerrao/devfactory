...
##For amazon s3 i am using dragondisk###

1)First Install this software amazon s3.
...
Enter the data Like data.... 
Accountname:devfactory
AccessKey: AKIAJ2IGHZQHCSJYT22A
SecretKey: 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
...
4)Run another terminal  at the same time.
  bundle exec s3ftp --config config/config.yml

5)Delete the file or directory click the delete link.

6)download the file click the download link  the file will be stores our application folder.(or) click on the file name also it will download.
